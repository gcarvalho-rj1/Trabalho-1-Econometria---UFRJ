#######################################################
# Projeto: Trabalho Econometria Questão 1
# Objetivo: Criar um scatterplot do log(salário) contra experiência
# Fazer uma regressão linear e plotar os valores preditos no gráfico
# Responsáveis: Mario (123674609), Guilherme (123671562) e Matheus (123443189)
# Data do projeto: Novembro de 2025

# input: data
# output: scatter_log(wage_homo)_experience_pred
#######################################################

# Carrega a base de dados
load(file.path(dir_input, "base_inicial.RData"))


# Realiza a regressão do log(salário homoscedástico) pela educação
regress_3 <- lm(log_wage_homo ~ experience, data = data)

# Cria os valores preditos
data$log_wage_pred <- predict(regress_3)

# 3) Gráfico
ggplot(data, aes(x = experience, y = log_wage_homo)) + geom_point(alpha = 0.6) +                         
  geom_line(aes(y = log_wage_pred), color = "red", size = 1.1) +                           
  labs(
    title = "Relação entre log(Salário) homoscedástico e Experiência (anos)",
    x = "Experiência",
    y = "log(Salário)",
    caption = "Linha vermelha: valores preditos pela regressão linear"
  ) +
  theme_minimal()


# Salva o gráfico
ggsave(
  filename = file.path(dir_output, "scatter_log(wage_homo)_experience_pred.png"), 
       width = 7, height = 5)
